...
class ArtContest(commands.Cog):
    ...
    async def get_next_submission_number(self) -> int:
        ...
        result = await self.db.submissions.find_one_and_update(
            {"_id": "submission_counter"}, {"$inc": {"count": 1}}, upsert=True, return_document=True
        )
        return result["count"]

    @nextcord.slash_command(name="art_contest", description="Submit your entry to the art contest!", guild_ids=[lib.SERVER_ID])
    async def art_contest(
        self,
        interaction: nextcord.Interaction,
        submission: nextcord.Attachment = nextcord.SlashOption(
            name="submission", description="Upload your art submission (image file only).", required=True
        ),
        ship_title: str = nextcord.SlashOption(name="ship_title", description="Enter the title of your ship.", required=True),
    ) -> None:
        ...
        if participant_role in interaction.user.roles:
            ...
            embed.description = ":x: You have already submitted an entry to the art contest!"
        ...
        await interaction.user.add_roles(participant_role, reason="Art contest submission")
        ...
        if art_contest_channel:
            embed = nextcord.Embed(title=f"Submission #{submission_number}: {ship_title}", color=config.EMBED_COLOR)
            embed.set_image(url=submission.url)
            await art_contest_channel.send(embed=embed)
        else:
            staff_channel = guild.get_channel(1261226647517003777)
            await staff_channel.send("<@961063229168164864> the art contest module is broken: the art contest logging thread couldn't be found.")

        # Log the user's submission in the log thread
        log_thread = guild.get_thread(ART_CONTEST_LOG_THREAD_ID)
        if log_thread:
            await log_thread.send(f"**Submission #{submission_number}**\n**Username:** {interaction.user.name}\n**User ID:** {interaction.user.id}")
        else:
            staff_channel = guild.get_channel(1261226647517003777)
            await staff_channel.send("<@961063229168164864> the art contest module is broken: the art contest logging thread couldn't be found.")
