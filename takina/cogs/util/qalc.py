...
class Qalc(commands.Cog):
    ...
    async def qalc_expression(self, expr: str):
    ...
    async def qalc_embed(self, expr: str):
        embed = nextcord.Embed(color=config.EMBED_COLOR)

        try:
            async with asyncio.timeout(config.QALC_TIMEOUT):
                result = await self.qalc_expression(expr)
    ...
    async def qalc(self, ctx: commands.Context, *, calculation: str):
        await ctx.reply(embed=await self.qalc_embed(calculation), mention_author=False)
    ...
    async def slash_qalc(
        ...
        *,
        ...
        await interaction.send(embed=await self.qalc_embed(calculation), ephemeral=True)
